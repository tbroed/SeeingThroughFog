...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, dataset_cfg, class_names, logger=None):
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict):
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            ...
            if 'COMPENSATE' in self.dataset_cfg:
                if self.dataset_cfg.COMPENSATE:
                    compensation = np.zeros(obj_points.shape)
                    compensation[:, :3] = np.array(self.dataset_cfg.COMPENSATE)
                    obj_points = obj_points + compensation
    ...
    def __call__(self, data_dict):
        ...
        for class_name, sample_group in self.sample_groups.items():
            ...
            if int(sample_group['sample_num']) > 0:
                sampled_dict = self.sample_with_fixed_number(class_name, sample_group, data_dict['mor'])
                ...
                if 'COMPENSATE' in self.dataset_cfg:
                    if self.dataset_cfg.COMPENSATE:
                        compensation = np.zeros(sampled_boxes.shape)
                        compensation[:, :3] = np.array(self.dataset_cfg.COMPENSATE)
                        sampled_boxes = sampled_boxes + compensation
